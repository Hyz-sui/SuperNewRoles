...
public static class RoleSelectHandler
{
    ...
    public static void SpawnBots()
    {
        if (ModeHandler.IsMode(ModeId.SuperHostRoles) && !ModeHandler.IsMode(ModeId.HideAndSeek, ModeId.VanillaHns))
        {
            ...
            int crewmate = 0;
            //ジャッカルがいるなら
            if (CustomOptionHolder.JackalOption.GetSelection() != 0 || JackalSeer.RoleOption.GetSelection() != 0)
            {
    ...
    public static void SetCustomRoles()
    {
        ...
        SetRoleDesync(JackalSeer.allPlayers, RoleTypes.Impostor);
        /*============インポスターにDesync============*/


        /*============エンジニアに役職設定============*/
        if (Jester.CanUseVentS) SetVanillaRole(Jester.allPlayers, RoleTypes.Engineer);
        ...
        SetVanillaRole(EvilSeer.allPlayers, RoleTypes.Shapeshifter, false);
        /*============シェイプシフター役職設定============*/
